...
if (isset($_POST['submit'])) {
   ...
   $update_profile_name = $conn->prepare("UPDATE `user` SET name = ? WHERE id = ?");
   $update_profile_name->execute([$name, $admin_id]);

   $empty_pass = 'da39a3ee5e6b4b0d3255bfef95601890afd80709'; // Empty password (no password entered)
   $prev_pass = $fetch_profile['password']; // Previous password from the fetched profile
